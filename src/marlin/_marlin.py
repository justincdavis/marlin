...
class Marlin:
    ...
    def __call__(self, frame: np.ndarray) -> list[tuple[int, tuple[int, int, int, int], float]]:
        ...
        if self._use_dnn or self._last_bboxs is None:
            ...
            if len(self._last_bboxs) == 0:
                self._use_dnn = True
                self._last_bboxs = prev_bboxs  # get the previous ones to reduce the flashing effect
            elif len(self._last_bboxs) == 1 and self._last_bboxs[0][0] == None:
                self._use_dnn = True
                self._last_bboxs = prev_bboxs  # get the previous ones to reduce the flashing effect
            else:
