...
class MultiBoxTracker:
    ...
    def init(self, frame: np.ndarray, detections: list[tuple[int, tuple[int, int, int, int], float]]) -> None:
        ...
        self._trackers = [
            LucasKanadeTracker() for _ in range(len(detections))
        ]
        for detection, tracker in zip(self._prev_dects, self._trackers):
    ...
    def run(self, frame: np.ndarray) -> list[tuple[int, tuple[int, int, int, int], float]]:
        ...
        new_dects = [
            (object_id, tracker.update(gray), confidence) 
            for (object_id, _, confidence), tracker in zip(self._prev_dects, self._trackers)
        ]
